    ...
    <div class="view-inner-mobile">
      <div class="need-scroll">
          ...
          <div class="oper-box">
            ...
            >
              <ul class="view-choice">
                <li
                  @click="goPDF(detailsMsg.coin_mobile_pdf, 3, detailsMsg.coin)"
                >
                  <!-- 查看研报 -->
            ...
            <base-button
              ...
              @click="goPDF(detailsMsg.mobile_pdf, 1, detailsMsg.id)"
            >
              <!-- 阅读研报 -->
              {{ $t("project_info.view_reports") }}
            </base-button>
        ...
        <EchartsBox
          ...
          :symbol="$route.query.name"
        />
        <!-- 代币分发规则 -->
        <CoinTable
          v-if="detailsMsg.is_tokens == 1 && detailsMsg.is_regular == 1"
        />
        <!-- 重大事件 -->
        <EventTable />
        <!-- 我的咨询 & 资讯 -->
        ...
        <!-- 同赛道竞品 -->
        <div
          class="compete-box-mobile"
          @click="competeBox = true"
          v-if="detailsMsg.competitor && detailsMsg.competitor.length > 0"
        >
          <p>
            <span class="iconfont icon-xianglintiaomu_adjacent-item"></span>
            <!-- 赛道竞品 -->
        ...
        <van-popup
          ...
          v-model="competeBox"
        >
          <div class="compete-list-mobile">
            <p class="compete-title">
              <span class="iconfont icon-xianglintiaomu_adjacent-item"></span>
              <!-- 赛道竞品 -->
              {{ $t("project_interface.sector") }}
            </p>
            <ul>
              <li
                v-for="(competeMobile, indexComM) in detailsMsg.competitor"
                :key="indexComM"
                @click="changeProMobile(competeMobile.id, competeMobile.symbol)"
              >
                <div>
                  <img :src="competeMobile.logo" alt="" />
                  <p>
                    {{ competeMobile.title
                    }}<span>{{ competeMobile.symbol }}</span>
                  </p>
                </div>
