  ...
  <div class="project-center">
    ...
    <div class="filter-box-new filter-box-new-pc" v-click-outside="closeFilter">
      ...
      <div class="filter-box-new-inner">
        ...
        <div class="title-filter">
          ...
          <el-checkbox v-model="attentionToUn" @change="attentionJSChange">
            <!-- 关注转不关注 -->
            {{ $t("project_info.change_follow") }}
          </el-checkbox>
          <el-checkbox v-model="unattentionToAt" @change="attentionJSChangeTwo">
            <!-- 不关注转关注 -->
      ...
      >
        ...
        <div class="level-one">
          ...
          <ul class="stop-click-ul" v-if="isChooseOne">
            <li
              class="stop-click"
              v-for="(i, indexOneRace) in raceList"
              :key="indexOneRace"
              @click.stop="
                raceList = [];
                levelOneText = i.name;
    ...
    <div class="row" v-loading="loadData" v-if="isHasData">
      <div
        ...
        v-for="(pro, indexPro) in proList"
        :key="indexPro"
      >
        <ProjectComponent :data="pro" />
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      attention: false, //关注
      unattention: false, //不关注
      checkAll: false, //全选
      raceList: [], //赛道列表
      coinList: [], //热币列表
      checkedRace: null, //选中赛道
      filterStatus: false, //筛选展开状态
      proList: [], //项目列表
      total: 1, //总页数
      defaultPagination: 1, //默认页码
      isWithPage: false, //是否需要页码
      isAttention: "", //是否关注  0——>否  1——>是
      loadData: false, //数据加载
      isHasData: true,
      mobileTabs: false,
      unCoin: false, //尚未发币
      ...
      levelTwoText: [],
      attentionToUn: false, //关注转不关注
      unattentionToAt: false, //不关注转关注
    };
  },
  watch: {
  ...
  methods: {
    ...
    //关注转不关注
    attentionJSChange(val) {
      if (val) {
        this.attention = false;
        this.unattention = false;
        this.unattention = false;
        this.unattentionToAt = false;
        this.isAttention = "3";
      } else {
        this.isAttention = "";
      }
    ...
    //不关注转关注
    attentionJSChangeTwo(val) {
      if (val) {
        this.attention = false;
        this.unattention = false;
        this.attentionToUn = false;
        this.isAttention = "4";
      } else {
        this.isAttention = "";
      }
    ...
    //项目赛道
    async getRaceList() {
      const result = await api.projectRace({ pid: this.levelOneID });
      if (result.length > 0) {
    ...
    checkRace(_index, _type) {
      ...
      this.checkedRace =
        _type == 1 ? this.levelOneID : this.raceList[_index].id;
    ...
    async getProList() {
      ...
      const proMsg = {
        ...
        is_attention: this.isAttention,
        is_tokens: this.unCoinType,
        page: this.defaultPagination,
        limit: 12,
      };
      const result = await api.projectList(proMsg);
      this.loadData = false;
